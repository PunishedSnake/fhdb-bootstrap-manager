...
static void scan_skip_hdd_settings(boot_chain_info_t *info)
{
    info->skip_hdd[0] =
...
static void scan_sysconf_partition(boot_chain_info_t *info)
{
    char *configuration;
    char value[96];
    ...
    if (configuration != NULL && fhdb_configuration != NULL &&
        read_text_file(fhdb_configuration, configuration,
                       TEXT_FILE_LIMIT) >= 0) {
        if (config_value(configuration, "LK_Auto_E1", value,
                         sizeof(value)) ||
            config_value(configuration, "LK_Auto_E2", value,
                         sizeof(value)) ||
            config_value(configuration, "LK_Auto_E3", value,
                         sizeof(value)))
    ...
    if (configuration != NULL && info->osdmenu_mbr_config &&
        read_text_file("pfs0:/osdmenu/OSDMBR.CNF", configuration,
                       TEXT_FILE_LIMIT) >= 0 &&
        config_value(configuration, "boot_auto", value, sizeof(value))) {
        snprintf(info->osdmenu_auto_path,
                 sizeof(info->osdmenu_auto_path), "%s", value);
        if (strcmp(value, "$PSBBN") == 0)
            info->osdmenu_auto_target = 1;
        else if (strcmp(value, "$HOSDSYS") == 0)
...
static void scan_system_partition(boot_chain_info_t *info)
{
...
static void render_boot_chain_report(const boot_chain_info_t *info)
{
    ...
    boot_report_length = 0;
    boot_report[0] = '\0';
    sha256_hex(info->payload_sha256, payload_hash);
    sha256_hex(info->kelf_sha256, kelf_hash);
    append_text(boot_report, sizeof(boot_report), &boot_report_length,
    ...
    for (port = 0; port < 2; port++) {
        ...
        for (i = 0; i < sizeof(folders) / sizeof(folders[0]); i++) {
            if (info->mc_folder_mask[port] & (1u << i)) {
                ...
                append_text(boot_report, sizeof(boot_report),
                            &boot_report_length, "  folder: %s\n", folders[i]);
                for (module = 0;
    ...
    append_text(boot_report, sizeof(boot_report), &boot_report_length,
                ...
                info->osdmenu_auto_target == 1 ? "$PSBBN" :
                info->osdmenu_auto_target == 2 ? "$HOSDSYS" :
                info->osdmenu_auto_target == 3 ? "custom" : "unavailable",
                info->osdmenu_auto_path[0] != '\0' ?
