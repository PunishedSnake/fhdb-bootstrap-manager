...
The active `dev/0.4.0-michishirube` branch is refactoring internals behind regression-gated module boundaries without changing Torii's write transaction. Read-only raw HDD acquisition now lives in `hdd_read.c`; payload/KELF fingerprinting is split between portable `boot_payload.c` and PS2-specific `boot_payload_ps2.c`. `boot_diagnostics_ps2.c` orchestrates the complete read-only evidence scan; `boot_report_session.c` owns the latest rendered report/session save result, `boot_report_ps2.c` owns `BOOTCHAIN.TXT` device persistence, and `session_log.c` owns bounded `HDDMAN.LOG` buffering, rotation, and flush policy. `header_backup.c` now owns the mandatory non-overwriting 1024-byte header backup/legacy lookup storage mechanics and exposes explicit per-slot diagnostics. `hdd_write.c` owns write-capable APA transport primitives; portable `bootstrap_transaction.c` plus its PS2 adapter own the post-confirmation payload-release/pointer sequencing. `main.c` retains validation, the decision that backup is mandatory, user confirmation, MagicGate signing, and error/UI policy. This work remains development-only until the branch completes its hardware-validation gate.
