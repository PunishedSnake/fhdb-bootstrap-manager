...
# Build the isolated Phase-5 transaction-workspace v2 experiment without
# changing default runtime sources. V2 extends v1 ownership backwards through
# execute_transaction() source admission so its fingerprint, COPY/source-hash
# and HDD verify phases borrow one 64 KiB / 64-byte-aligned EE workspace.
# The pre-confirmation UI fingerprint remains helper-owned and short-lived.
...
python3 "$ROOT/tools/allocation_inventory.py" \
    --root "$ROOT" \
    --output "$ROOT/ALLOCATION_INVENTORY_TX_WORKSPACE_V2.json"

build_variant()
{
    profile=$1
    label=$2
    elf="PS2_HDD_BOOTSTRAP_MANAGER_TX_WORKSPACE_V2_PROFILE_${label}.ELF"
    map="PS2_HDD_BOOTSTRAP_MANAGER_TX_WORKSPACE_V2_PROFILE_${label}.map"
    irx="HDL_STREAM_TX_WORKSPACE_V2_PROFILE_${label}.irx"
    audit="OPTIMIZATION_AUDIT_TX_WORKSPACE_V2_PROFILE_${label}.txt"
    provenance="BENCHMARK_PROVENANCE_TX_WORKSPACE_V2_PROFILE_${label}.yml"
